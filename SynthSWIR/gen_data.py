...
# This code randomly samples the set of geotifs and then saves data reshaped
# as 1-D vectors in a csv that will be used for the tensorflow workflow
...
def pull_bands(img, band_ind, x_vals, y_vals):
    ...
    Gets a single (x,y) point for a specific band.
...
def get_sample(file_name, num_pts):
...
def create_training(file_dir, num_pts=5000):
    ...
    num_pts : `int`
        Number of samples to collect per image, default is 5000
